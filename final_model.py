...
class Model(nn.Module):
    ...
    - hashed TF-IDF logistic branch (see model_source/model_branch_hash_tfidf.py)
    - vocab word+char TF-IDF logistic branch (see model_source/model_branch_word_char_tfidf.py)
    """
    ...
    def __init__(self, *args, **kwargs) -> None:
        ...
        # Ensemble controls
        self.register_buffer("hash_weight", torch.tensor(0.65, dtype=torch.float32))
        self.register_buffer("vocab_char_weight", torch.tensor(0.35, dtype=torch.float32))
        self.register_buffer("decision_threshold", torch.tensor(0.5, dtype=torch.float32))

        # Hashed branch params
        self.hash_num_features = 1 << 15
